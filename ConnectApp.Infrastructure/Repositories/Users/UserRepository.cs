...
namespace ConnectApp.Infrastructure.Repositories.Users
{
    public class UserRepository : IUserRepository
    {
        ...
        public async Task<SignInResult> CreatesUserAsync(User user)
        {
            var commandText = new StringBuilder()
            ...
            try
            {
                if (user == null)
                {
                    ...
                    var UserResult = new SignInResult
                    {
                        Error = false,
        ...
        //        .AppendLine("    [AccountId] = @account_id,") // Parâmetro em snake_case
        ...
        private static void AddUser(SqlCommand cm, User user)
        {
            ...
            cm.Parameters.AddWithValue("@Email", string.IsNullOrWhiteSpace(user.Email) ? DBNull.Value : user.Email);
            cm.Parameters.AddWithValue("@Phone", string.IsNullOrWhiteSpace(user.Phone) ? DBNull.Value : user.Phone);
            ...
            cm.Parameters.AddWithValue("@AccountId", user.AccountId == Guid.Empty ? DBNull.Value : user.AccountId);
            cm.Parameters.AddWithValue("@AccountName", string.IsNullOrWhiteSpace(user.AccountName) ? DBNull.Value : user.AccountName);
        ...
        //    cm.Parameters.AddWithValue("@AccountId", user.AccountId == Guid.Empty ? (object)DBNull.Value : user.AccountId);
        ...
        private static User GetDataRecord(SqlDataReader reader)
        {
            return new User
            {
                ...
                IsActive = reader.GetNullableInt("IS_ACTIVE") ?? 0,
        ...
        private void AddUserUpdateParameters(SqlCommand cm, User user)
        {
            // Parâmetros
            cm.Parameters.AddWithValue("@Id", user.Id);
            cm.Parameters.AddWithValue("@Code", user.Code);
            ...
            cm.Parameters.AddWithValue("@Email", user.Email ?? (object)DBNull.Value);
            cm.Parameters.AddWithValue("@Phone", user.Phone ?? (object)DBNull.Value);
        ...
        public async Task<SignInResult> GetUserByUsernameAsync(string accessKey)
        {
        ...
        Task<User> IUserRepository.CreatesUserAsync(User user)
        {
            throw new NotImplementedException();
        }

        Task<User> IUserRepository.CreateUserAsync(User user)
        {
            throw new NotImplementedException();
        }

        // Este método adiciona os parâmetros ao comando para a atualização do usuário
        ...
        //    command.Parameters.Add(new SqlParameter("@account_id", SqlDbType.UniqueIdentifier) { Value = user.AccountId ?? (object)DBNull.Value });
        ...
        //        { Value = (user.AccountId == Guid.Empty ? (object)DBNull.Value : user.AccountId ?? (object)DBNull.Value) });
